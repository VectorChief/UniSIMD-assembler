...
#include "rtarch_pHB_128x1v2.h"
#include "rtarch_pQF_128x1v2.h"
...
/*
 * rtarch_p64_128x1v2.h: Implementation of POWER fp64 VSX3 instructions.
 *
...
#if RT_ENDIAN == 0 && RT_ELEM_COMPAT_VMX == 0
#define SBF(x)  x
#define SBX(x)
#else /* RT_ENDIAN, RT_ELEM_COMPAT_VMX */
#define SBF(x)
#define SBX(x)  x
#endif /* RT_ENDIAN, RT_ELEM_COMPAT_VMX */
...
#if RT_ENDIAN == 0 && RT_ELEM_COMPAT_VMX != 0 && 0
#define SJF(x)  x
#define SJX(x)
#else /* RT_ENDIAN, RT_ELEM_COMPAT_VMX */
#define SJF(x)
...
#define elmjx_st(XS, MD, DD) /* 1st elem as in mem with SIMD load/store */  \
    ...
    SBF(EMITW(0xF0000257 | MXM(TmmM,    REG(XS), REG(XS))))                 \
    SBF(EMITW(0x00000000 | MPM(TmmM,    MOD(MD), VAL(DD), B1(DD), V1(DD)))) \
    SBX(EMITW(0x00000000 | MPM(REG(XS), MOD(MD), VAL(DD), B1(DD), V1(DD))))
...
#define cuzjs_rr(XD, XS)     /* round towards zero */                       \
        EMITW(0xF0000723 | MXM(REG(XD), 0x00,    REG(XS)))

#define cuzjs_ld(XD, MS, DS) /* round towards zero */                       \
        AUW(SIB(MS),  EMPTY,  EMPTY,    MOD(MS), VAL(DS), C2(DS), EMPTY2)   \
        EMITW(0x00000000 | MPM(TmmM,    MOD(MS), VAL(DS), B2(DS), P2(DS)))  \
    SHF(EMITW(0xF0000257 | MXM(TmmM,    TmmM,    TmmM)))                    \
        EMITW(0xF0000723 | MXM(REG(XD), 0x00,    TmmM))
...
#define movts_ld(XD, MS, DS)                                                \
        ...
        EMITW(0x00000000 | MPM(REG(XD), MOD(MS), VAL(DS), B1(DS), K1(DS)))

#define movts_st(XS, MD, DD)                                                \
        AUW(SIB(MD),  EMPTY,  EMPTY,    MOD(MD), VAL(DD), C1(DD), EMPTY2)   \
        EMITW(0x00000000 | MPM(REG(XS), MOD(MD), VAL(DD), B1(DD), V1(DD)))
...
#define addts3ld(XD, XS, MT, DT)                                            \
        ...
        EMITW(0x00000000 | MPM(TmmM,    MOD(MT), VAL(DT), B1(DT), K1(DT)))  \
...
#define sqrts_ld(XD, MS, DS)                                                \
        AUW(SIB(MS),  EMPTY,  EMPTY,    MOD(MS), VAL(DS), C1(DS), EMPTY2)   \
        EMITW(0x00000000 | MPM(TmmM,    MOD(MS), VAL(DS), B1(DS), K1(DS)))  \
