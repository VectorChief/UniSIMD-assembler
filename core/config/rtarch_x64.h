...
#define arjzx_rx(RG, OP, cc, lb)                                            \
        AR1(W(RG), VAL(OP), z, TYP(OP), Zrx)                                \
        CMJ(cc, lb)

#define arjzx_mx(MG, DG, OP, cc, lb)                                        \
        AR2(W(MG), W(DG), VAL(OP), z, TYP(OP), Zmx)                         \
        CMJ(cc, lb)

#define arjzx_ri(RG, IS, OP, cc, lb)                                        \
        AR2(W(RG), W(IS), VAL(OP), z, TYP(OP), Zri)                         \
        CMJ(cc, lb)

#define arjzx_mi(MG, DG, IS, OP, cc, lb)                                    \
        AR3(W(MG), W(DG), W(IS), VAL(OP), z, TYP(OP), Zmi)                  \
        CMJ(cc, lb)

#define arjzx_rr(RG, RS, OP, cc, lb)                                        \
        AR2(W(RG), W(RS), VAL(OP), z, TYP(OP), Zrr)                         \
        CMJ(cc, lb)

#define arjzx_ld(RG, MS, DS, OP, cc, lb)                                    \
        AR3(W(RG), W(MS), W(DS), VAL(OP), z, TYP(OP), Zld)                  \
        CMJ(cc, lb)

#define arjzx_st(RS, MG, DG, OP, cc, lb)                                    \
        AR3(W(RS), W(MG), W(DG), VAL(OP), z, TYP(OP), Zst)                  \
        CMJ(cc, lb)
