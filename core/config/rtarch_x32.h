...
#define arjwx_rx(RG, OP, cc, lb)                                            \
        AR1(W(RG), VAL(OP), w, TYP(OP), Zrx)                                \
        CMJ(cc, lb)

#define arjwx_mx(MG, DG, OP, cc, lb)                                        \
        AR2(W(MG), W(DG), VAL(OP), w, TYP(OP), Zmx)                         \
        CMJ(cc, lb)

#define arjwx_ri(RG, IS, OP, cc, lb)                                        \
        AR2(W(RG), W(IS), VAL(OP), w, TYP(OP), Zri)                         \
        CMJ(cc, lb)

#define arjwx_mi(MG, DG, IS, OP, cc, lb)                                    \
        AR3(W(MG), W(DG), W(IS), VAL(OP), w, TYP(OP), Zmi)                  \
        CMJ(cc, lb)

#define arjwx_rr(RG, RS, OP, cc, lb)                                        \
        AR2(W(RG), W(RS), VAL(OP), w, TYP(OP), Zrr)                         \
        CMJ(cc, lb)

#define arjwx_ld(RG, MS, DS, OP, cc, lb)                                    \
        AR3(W(RG), W(MS), W(DS), VAL(OP), w, TYP(OP), Zld)                  \
        CMJ(cc, lb)

#define arjwx_st(RS, MG, DG, OP, cc, lb)                                    \
        AR3(W(RS), W(MG), W(DG), VAL(OP), w, TYP(OP), Zst)                  \
        CMJ(cc, lb)
