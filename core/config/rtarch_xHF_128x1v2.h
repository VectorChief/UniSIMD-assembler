...
#define fmags_rr(XG, XS, XT)                                                \
        EFX(RXB(XG), RXB(XT), REN(XS), 0, 1, 2) EMITB(0xB8)                 \
...
#define fmsgs_rr(XG, XS, XT)                                                \
        EFX(RXB(XG), RXB(XT), REN(XS), 0, 1, 2) EMITB(0xBC)                 \
        MRM(REG(XG), MOD(XT), REG(XT))
...
#define cvtgx_rr(XD, XS)                                                    \
        EFX(RXB(XD), RXB(XS),    0x00, 0, 3, 1) EMITB(0x7D)                 \
        MRM(REG(XD), MOD(XS), REG(XS))

#define cvtgx_ld(XD, MS, DS)                                                \
    ADR EFX(RXB(XD), RXB(MS),    0x00, 0, 3, 1) EMITB(0x7D)                 \
        MRM(REG(XD), MOD(MS), REG(MS))                                      \
        AUX(SIB(MS), CMD(DS), EMPTY)
...
#define cuzgs_rr(XD, XS)     /* round towards zero */                       \
        EFX(RXB(XD), RXB(XS),    0x00, 0, 0, 1) EMITB(0x7C)                 \
        MRM(REG(XD), MOD(XS), REG(XS))

#define cuzgs_ld(XD, MS, DS) /* round towards zero */                       \
    ADR EFX(RXB(XD), RXB(MS),    0x00, 0, 0, 1) EMITB(0x7C)                 \
        MRM(REG(XD), MOD(MS), REG(MS))                                      \
        AUX(SIB(MS), CMD(DS), EMPTY)
...
#define cutgs_rr(XD, XS)                                                    \
        EFX(RXB(XD), RXB(XS),    0x00, 0, 0, 1) EMITB(0x7D)                 \
        MRM(REG(XD), MOD(XS), REG(XS))

#define cutgs_ld(XD, MS, DS)                                                \
    ADR EFX(RXB(XD), RXB(MS),    0x00, 0, 0, 1) EMITB(0x7D)                 \
        MRM(REG(XD), MOD(MS), REG(MS))                                      \
        AUX(SIB(MS), CMD(DS), EMPTY)
...
#define fmans_rr(XG, XS, XT)                                                \
        EFX(RXB(XG), RXB(XT), REN(XS), 0, 1, 2) EMITB(0xB9)                 \
        MRM(REG(XG), MOD(XT), REG(XT))
...
#define fmsns_rr(XG, XS, XT)                                                \
        EFX(RXB(XG), RXB(XT), REN(XS), 0, 1, 2) EMITB(0xBD)                 \
        MRM(REG(XG), MOD(XT), REG(XT))
